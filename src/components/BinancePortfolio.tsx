...
export default function BinancePortfolio() {
  const { connected, positions, totalValue, loading, refreshPortfolio } = useBinance();
  ...
  return (
    <div className="bg-gray-900/50 border border-gray-800 rounded-2xl overflow-hidden">
      <div className="px-6 py-4 border-b border-gray-800 flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-white">Binance Portfolio</h2>
          <p className="text-gray-500 text-sm mt-1">
            Total: <span className="text-white font-mono">${totalValue.toLocaleString(undefined, { maximumFractionDigits: 2 })}</span>
          </p>
        </div>
        <button
                    ...
                    <div className="flex items-center justify-end gap-2">
                      <div className="w-16 h-1.5 bg-gray-800 rounded-full overflow-hidden">
                        <div
                          ...
                          style={{ width: `${totalValue ? (pos.usdValue / totalValue) * 100 : 0}%` }}
                        />
                      </div>
                      <span className="text-gray-400 text-sm font-mono">
                        {totalValue ? ((pos.usdValue / totalValue) * 100).toFixed(1) : 0}%
                      </span>
                    </div>
                  </td>
                </tr>
