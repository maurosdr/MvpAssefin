          ...
          <div key={i} className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <div className="w-16 h-1.5 bg-gray-700 rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${i === 0 ? 'bg-green-500' : i === 1 ? 'bg-yellow-500' : 'bg-gray-500'}`}
                  style={{ width: `${outcome.price * 100}%` }}
                />
              </div>
              <span className={`font-mono text-xs font-medium ${i === 0 ? 'text-green-400' : i === 1 ? 'text-yellow-400' : 'text-gray-400'}`}>
