...
export default function TradingChat({ onBacktestFound, isRunningBacktest, keysConfigured, onOpenSettings }: TradingChatProps) {
  ...
  const [lastBacktestStrategy, setLastBacktestStrategy] = useState<BacktestStrategy | null>(null);
  ...
  const handleSend = useCallback(async () => {
    ...
    try {
      ...
      if (strategy) {
        setLastBacktestStrategy(strategy);
  ...
  return (
    <div className="modern-card flex flex-col h-[720px]">
      ...
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto p-4 space-y-4 min-h-0">
        ...
        {lastBacktestStrategy && !isRunningBacktest && (
          <div className="sticky bottom-0 flex justify-center pt-2 pb-1">
            <button
              onClick={() => onBacktestFound(lastBacktestStrategy)}
              className="flex items-center gap-2 px-5 py-2.5 bg-[var(--accent)] hover:bg-[var(--accent-hover)] text-[var(--text-inverse)] rounded-full text-sm font-bold shadow-lg transition-all active:scale-95"
            >
